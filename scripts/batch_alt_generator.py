...
def is_heading(line):
    if line.startswith(' ') or line.startswith('\t'):
        return False
    s = line.strip()
    return s.startswith('#') and any(s.startswith(p) for p in HEADING_PREFIXES)
...
def wrap_script_in_function(code):
...
def gen_function_alts(code):
    ...
    wrapped, _ = wrap_script_in_function(code)
    if wrapped and compile_check(wrapped):
        alts = [wrapped]
        while_version = convert_for_to_while(wrapped)
        if while_version and compile_check(while_version):
            alts.append(while_version)
        if len(alts) == 2:
            return alts, None
