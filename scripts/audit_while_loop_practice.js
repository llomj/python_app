...
try {
  ...
  const failures = [];
  if (exercises.length !== 150) failures.push(`expected 150 exercises, found ${exercises.length}`);
  const expectedIds = [
    ...Array.from({ length: 100 }, (_, index) => 2381 + index),
    ...Array.from({ length: 50 }, (_, index) => 5658 + index),
  ];
  if (JSON.stringify(exercises.map(item => item.id)) !== JSON.stringify(expectedIds)) failures.push('IDs are not exactly 2381-2480 followed by 5658-5707');
  ...
  for (const exercise of exercises) {
    ...
    if (exercise.id >= 5658 && exercise.category !== 'While Loop Easy') failures.push(`${exercise.id}: new exercise is not Easy`);
    if (exercise.id >= 5658 && !/Difficulté\s*:\s*Facile/i.test(localized?.description || '')) failures.push(`${exercise.id}: new French exercise is not Easy`);
  }
