...
function loadTsExports(fileName) {
  ...
  const sandbox = {
    ...
    require: (specifier) => {
      ...
      if (specifier === './easyConceptPracticeExercises') return loadTsExports('easyConceptPracticeExercises.ts');
      if (specifier === './easyConceptPracticeGraders') return loadTsExports('easyConceptPracticeGraders.ts');
      if (specifier === './conceptLevelExpansionExercises') return loadTsExports('conceptLevelExpansionExercises.ts');
      if (specifier === './conceptLevelExpansionGraders') return loadTsExports('conceptLevelExpansionGraders.ts');
      if (specifier === './coreConceptExpansionExercises') return loadTsExports('coreConceptExpansionExercises.ts');
      if (specifier === './coreConceptExpansionGraders') return loadTsExports('coreConceptExpansionGraders.ts');
      if (specifier === './supplementalConceptExercises') return loadTsExports('supplementalConceptExercises.ts');
      if (specifier === './supplementalConceptGraders') return loadTsExports('supplementalConceptGraders.ts');
      return {};
    },
  };
...
def kwargs_metamorphic_cases(function_names, tests):
    if not tests:
        return []
    for case in tests:
        if not isinstance(case.get("kwargs"), dict) or not is_simple_case({**case, "kwargs": {}}):
            return []
    name_set = set(function_names)
...
def function_script_test_cases(function_names, tests):
    ...
    cases += kwargs_metamorphic_cases(function_names, tests)
    for args, expected in metamorphic_cases(function_names, tests):
