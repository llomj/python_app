...
const CONCEPT_DOCS: Record<ConceptModeId, string> = {
    'concept:builtins': `# Simple Explanation:
Built-ins are tools that come with Python automatically. You do not need to import anything to use them. They are always available, like print, len, sum, max, min, range, and type.

# Intermediate Explanation:
Python has a large standard library of built-in functions that handle common tasks. They work on many data types. len counts items, sum adds numbers, max and min find extremes, sorted returns an ordered copy, and range creates number sequences. You call them by name followed by parentheses.

# In-Depth Explanation:
Built-ins are defined in the builtins module, which Python searches last in the LEGB scope order. They are implemented in C for speed. Some accept key functions, default values, or multiple iterables. Knowing built-ins well lets you write shorter, faster Python without extra imports.

# Common Methods:
print(value) — display output
len(iterable) — count items
sum(iterable) — add numbers
...
print(list(enumerate('ab'))) # [(0, 'a'), (1, 'b')]`,
    ...
    'concept:closures': `# Simple Explanation:
...
# Intermediate Explanation:
When a function is defined inside another function, the inner function can read variables from the outer function. If the inner function is returned or stored, it keeps a reference to those variables. This bundle of function plus captured variables is called a closure.

# In-Depth Explanation:
Closures work because Python functions carry a __closure__ attribute that holds references to free variables from enclosing scopes. They are used for factories, decorators, and callbacks. The nonlocal keyword lets a closure modify an enclosing variable.
...
print(triple(5))  # 15`,
    ...
    'concept:comprehensions': `# Simple Explanation:
Comprehensions are a short way to create a list, dictionary, or set by looping over something in a single line.

# Intermediate Explanation:
A list comprehension looks like [expression for item in iterable if condition]. It can filter and transform items. Dictionary comprehensions use {key: value for ...} and set comprehensions use {value for ...}. They are usually faster and more readable than a full for-loop.

# In-Depth Explanation:
Comprehensions create a new scope for their loop variable. They can be nested but avoid making them too complex. Generator expressions use parentheses and produce values lazily, saving memory for large sequences.
...
word_lengths = {w: len(w) for w in ['cat', 'dog']}
print(word_lengths)  # {'cat': 3, 'dog': 3}`,

    'concept:conditionals': `# Simple Explanation:
Conditionals let your program make decisions. You use if, elif, and else to run different code depending on whether something is true or false.

# Intermediate Explanation:
An if statement checks a boolean condition. elif adds more checks. else runs when none match. Python also has a one-line ternary expression: value_if_true if condition else value_if_false.

# In-Depth Explanation:
Conditions are evaluated in order and short-circuit. Truthy values include non-zero numbers, non-empty collections, and non-None objects. Falsy values include 0, empty collections, None, and False. Indentation defines which code belongs to which branch.
...
print(status)  # pass`,
    ...
    'concept:dictionaries': `# Simple Explanation:
A dictionary stores pairs of keys and values. You can look up a value quickly if you know its key.

# Intermediate Explanation:
Dictionaries are written with curly braces: {'name': 'Alice', 'age': 30}. Keys must be hashable, usually strings or numbers. Values can be anything. Use dict[key] to access or d.get(key, default) for safe access.

# In-Depth Explanation:
Dictionaries are implemented as hash tables, so lookups are very fast on average. Insertion order is preserved in Python 3.7+. Keys must be immutable. Common patterns include counting with a default of 0 and grouping values into lists.
...
for key, value in person.items():
    ...
    'concept:exceptions': `# Simple Explanation:
Exceptions are errors that happen while a program runs. You can catch them with try and except to handle problems gracefully.

# Intermediate Explanation:
Code that might fail goes inside try. If an exception occurs, Python jumps to the matching except block. You can use finally for cleanup, else for code that runs only if no exception happened, and raise to trigger your own errors.

# In-Depth Explanation:
Exceptions are objects. You can catch specific types like ValueError or FileNotFoundError, or catch Exception as a base class. Bare except: catches everything including KeyboardInterrupt, so prefer specific types. Custom exceptions inherit from Exception.
...
try:
    ...
    'concept:files': `# Simple Explanation:
Files let your program read from and write to the disk. You open a file, do something with it, and then close it.

# Intermediate Explanation:
Use open(path, mode) where mode is 'r' for read, 'w' for write, or 'a' for append. The with statement automatically closes the file. read() gets the whole file, readlines() gets a list of lines, and write() saves text.

# In-Depth Explanation:
Files are iterable line by line. For large files, iterate directly instead of loading everything. Text mode handles encoding; binary mode handles bytes. JSON and CSV have dedicated modules for structured data.
...
with open('data.txt') as f:
    ...
    'concept:for_loops': `# Simple Explanation:
A for loop repeats code for every item in a sequence, like a list, string, or range of numbers.

# Intermediate Explanation:
for item in iterable runs the body once per item. range(n) gives numbers 0 to n-1. enumerate gives both index and value. You can loop over strings, lists, dictionaries, files, and any iterable.

# In-Depth Explanation:
For loops use iterators under the hood. break exits early, continue skips the rest of the current iteration, and else runs only if the loop completed without break. Nested loops are common for grids and patterns.
...
for i, word in enumerate(words):
    print(i, word)  # 0 cat, 1 dog

for char in 'hi':
    print(char)  # h, i`,

    'concept:functions': `# Simple Explanation:
A function is a reusable block of code that performs a task. You define it once with def and call it whenever you need it.

# Intermediate Explanation:
Functions can accept inputs called parameters and return outputs. They help organize code, avoid repetition, and make programs easier to read. Default values let parameters be optional. Docstrings explain what a function does.

# In-Depth Explanation:
Functions are first-class objects in Python. They can be passed as arguments, returned from other functions, and stored in variables. Scope rules determine which variables a function can see. args and kwargs handle flexible argument lists.
...
print(add(3, 2)) # 5`,
    ...
    'concept:generators': `# Simple Explanation:
A generator is a special function that produces values one at a time instead of creating a big list all at once.

# Intermediate Explanation:
Generators use the yield keyword. Each time yield runs, the function pauses and remembers its state. The next call resumes where it left off. This saves memory when working with large sequences.

# In-Depth Explanation:
Generator functions return generator objects. You can iterate over them once. Generator expressions look like list comprehensions but use parentheses. They are lazy and useful for pipelines, infinite sequences, and reading large files.
...
squares = (x*x for x in range(1000000))  # memory efficient`,
    ...
    'concept:input_output': `# Simple Explanation:
Input and output are how a program talks to the user. input() reads text from the keyboard and print() displays text on the screen.

# Intermediate Explanation:
input() always returns a string, so convert it with int(), float(), or str() when needed. print() can show multiple values separated by spaces. f-strings let you embed variables directly inside strings.

# In-Depth Explanation:
Input may contain trailing newlines or extra spaces; use strip() to clean it. print accepts sep and end parameters to control formatting. For files, use open() and read/write methods.
...
print('a', 'b', sep='-', end='!')  # a-b!`,
    ...
    'concept:lambdas': `# Simple Explanation:
A lambda is a small, anonymous function written in one line. It is useful for short operations.

# Intermediate Explanation:
Lambdas use the syntax lambda arguments: expression. They can have many arguments but only one expression. They are often used with map, filter, sorted(key=...), and other functions that need a quick helper.

# In-Depth Explanation:
Lambdas are limited to a single expression and cannot contain statements like assignments or loops. For anything complex, use a normal def function. They are closures and can capture surrounding variables.
...
print(list(filter(lambda x: x > 2, nums)))  # [3, 4]`,
    ...
    'concept:lists': `# Simple Explanation:
A list is an ordered collection of items. You can add, remove, and change items because lists are mutable.

# Intermediate Explanation:
Lists are written with square brackets: [1, 2, 3]. Items are accessed by index starting at 0. Slicing lets you get sublists. Methods like append, extend, pop, and sort change the list in place.

# In-Depth Explanation:
Lists can hold mixed types and nested lists. They are dynamic arrays with amortized O(1) appends. Indexing out of range raises IndexError. List comprehensions provide a concise way to create and transform lists.
...
print(fruits)  # ['apple', 'banana', 'cherry']`,
    ...
    'concept:math': `# Simple Explanation:
Python can do arithmetic and work with numbers using operators like +, -, *, /, and %.

# Intermediate Explanation:
The % operator gives the remainder after division. // does integer division. ** raises a number to a power. The math module provides functions like sqrt, ceil, floor, factorial, gcd, and trig functions.

# In-Depth Explanation:
Python integers have arbitrary precision. Floating point numbers have precision limits, so comparisons need care. The statistics module handles mean, median, and standard deviation. For exact fractions, use the fractions module.
...
print(math.gcd(12, 8)) # 4`,
    ...
    'concept:methods': `# Simple Explanation:
A method is a function that belongs to an object. You call it using dot notation, like text.upper().

# Intermediate Explanation:
Different types have different methods. Strings have upper(), split(), and replace(). Lists have append(), sort(), and count(). Dictionaries have keys(), values(), and get(). Methods often modify the object or return useful information.

# In-Depth Explanation:
Methods are functions stored as attributes on objects. Some methods change the object in place, while others return a new value. Chaining methods like text.strip().upper() is common. Understanding available methods saves time and makes code idiomatic.
...
items.append(4)
print(items)             # [1, 2, 2, 3, 4]`,

    'concept:modules': `# Simple Explanation:
A module is a file containing Python code. You can reuse its functions and variables by importing it into your program.

# Intermediate Explanation:
Use import module_name to bring in a whole module, or from module_name import function_name to bring in specific items. The standard library includes modules like math, random, datetime, os, and json.

# In-Depth Explanation:
Modules are loaded once and cached in sys.modules. You can create your own modules by saving code in a .py file. Packages are folders of modules with an __init__.py file. Avoid naming your files the same as standard library modules.
...
print(datetime.date.today())`,
    ...
    'concept:oop': `# Simple Explanation:
Object-oriented programming lets you create blueprints called classes. A class defines data and behavior, and objects are built from that blueprint.

# Intermediate Explanation:
A class groups variables called attributes and functions called methods. __init__ is the constructor that runs when an object is created. self refers to the current object. Inheritance lets one class reuse another class's code.

# In-Depth Explanation:
OOP models real-world things. Encapsulation hides internal details. Polymorphism lets different classes respond to the same method call. Special methods like __str__ and __len__ integrate with Python syntax.
...
print(buddy.bark())  # Buddy says woof!`,
    ...
    'concept:operators': `# Simple Explanation:
Operators are symbols that perform actions on values. Examples include +, -, *, /, ==, and, and or.

# Intermediate Explanation:
Arithmetic operators do math. Comparison operators compare values and return True or False. Logical operators combine conditions. Assignment operators like += update a variable. The in operator checks membership.

# In-Depth Explanation:
Operators have precedence rules that determine evaluation order. Parentheses override precedence. Comparison operators can be chained, like 1 < x < 10. The is operator checks identity, while == checks equality.
...
print(x)  # 7`,
    ...
    'concept:patterns': `# Simple Explanation:
Pattern problems ask you to print shapes like triangles, pyramids, or grids using loops and characters.

# Intermediate Explanation:
Patterns usually need nested loops. The outer loop controls rows, and the inner loops control spaces and stars. Understanding the relationship between row number and character count is the key to solving them.

# In-Depth Explanation:
Break the pattern into rows and columns. Count how many spaces and symbols each row needs. Use range to generate the right numbers. For hollow patterns, only print symbols at the borders.
...
# *******`,
    ...
    'concept:recursion': `# Simple Explanation:
Recursion is when a function calls itself to solve a smaller version of the same problem.

# Intermediate Explanation:
Every recursive function needs a base case that stops the recursion and a recursive case that moves closer to the base case. Recursion is natural for problems like factorials, Fibonacci, and tree traversal.

# In-Depth Explanation:
Each recursive call adds a new stack frame. Too many calls can cause a RecursionError. Recursion often mirrors mathematical definitions but can be less efficient than iteration. Memoization can speed recursive solutions.
...
countdown(3)  # 3, 2, 1`,
    ...
    'concept:regex': `# Simple Explanation:
Regular expressions are patterns used to search, match, and manipulate text. Python's re module provides regex support.

# Intermediate Explanation:
Common patterns include . for any character, * for zero or more, + for one or more, ^ for start, $ for end, and \\d for digits. re.search finds a match anywhere, re.match checks the start, and re.findall returns all matches.

# In-Depth Explanation:
Use raw strings r'...' for regex patterns so backslashes are not interpreted by Python. Groups with parentheses capture parts of a match. re.sub replaces matches. Compile complex patterns for reuse.
...
print(re.search(r'\\d{3}', text).group())  # 123
print(re.findall(r'\\d+', text))           # ['123', '456', '7890']
print(re.sub(r'\\d', '#', text))           # My number is ###-###-####`,

    'concept:sets': `# Simple Explanation:
A set is an unordered collection of unique items. It automatically removes duplicates.

# Intermediate Explanation:
Sets are written with curly braces: {1, 2, 3}. They support operations like union, intersection, difference, and symmetric difference. Sets are useful for membership tests and removing duplicates from a list.

# In-Depth Explanation:
Sets are implemented as hash tables, so membership tests are fast. Set elements must be hashable. Frozenset is an immutable version. Set operations can be done with operators |, &, -, ^ or with methods.

# Common Methods:
add(item) — add one item
remove(item) — remove item, error if missing
discard(item) — remove if present
union(other) or | — all items from both
intersection(other) or & — items in both
difference(other) or - — items only in first
...
print(a & b)  # {3}`,
    ...
    'concept:sorting': `# Simple Explanation:
Sorting arranges items in order, usually from smallest to largest or alphabetically.

# Intermediate Explanation:
sorted(iterable) returns a new sorted list without changing the original. list.sort() sorts the list in place. Both accept a key function that decides how to compare items, and reverse=True for descending order.

# In-Depth Explanation:
Python uses Timsort, a stable hybrid algorithm. The key function is called once per item, making sorts efficient. Common keys include len for strings, str.lower for case-insensitive sorting, and lambda x: x[1] for tuples.
...
print(sorted(pairs, key=lambda x: x[1]))  # [('b', 1), ('a', 3)]`,
    ...
    'concept:strings': `# Simple Explanation:
A string is a sequence of characters, like a word or sentence. Strings are written inside quotes.

# Intermediate Explanation:
Strings are immutable, so methods return new strings instead of changing the original. You can concatenate with +, repeat with *, and access characters by index. Slicing extracts substrings.

# In-Depth Explanation:
Python strings are Unicode by default. f-strings embed expressions. Escape sequences like \\n and \\t represent special characters. Methods like split, join, strip, replace, and startswith are used constantly.

# Common Methods:
upper() / lower() — change case
strip() — remove surrounding whitespace
split(sep) — break into list
join(iterable) — combine into string
...
print('hello'.startswith('he'))  # True`,
    ...
    'concept:tuples': `# Simple Explanation:
A tuple is an ordered collection of items, like a list, but it cannot be changed after creation.

# Intermediate Explanation:
Tuples are written with parentheses: (1, 2, 3). They are immutable, so they are safe to use as dictionary keys or set elements. Tuple unpacking lets you assign multiple variables at once.

# In-Depth Explanation:
Tuples are faster and use less memory than lists. Named tuples from collections give field names to tuple elements. Immutability makes tuples useful for fixed records and function returns.
...
print(low, high)  # 1 4`,
    ...
    'concept:type_conversion': `# Simple Explanation:
Type conversion changes a value from one type to another, like turning a string into a number.

# Intermediate Explanation:
int(), float(), str(), bool(), list(), tuple(), and set() convert values. int('5') becomes 5. str(5) becomes '5'. Some conversions fail if the value is invalid, like int('hello').

# In-Depth Explanation:
Implicit conversion happens automatically in some operations, like adding int and float. Explicit conversion is safer and clearer. bool conversion treats 0, empty collections, None, and False as falsy. Everything else is truthy.
...
print(bool('hello'))   # True`,
    ...
    'concept:variables': `# Simple Explanation:
A variable is a name that stores a value. You create one by writing name = value.

# Intermediate Explanation:
Variables can hold numbers, strings, lists, and other objects. The = sign assigns a value, it does not mean equality. Variable names should be descriptive and cannot start with a number.

# In-Depth Explanation:
Variables are references to objects in memory. Assigning one variable to another copies the reference, not the object. This matters for mutable objects like lists. Scope rules determine where a variable is visible.
...
print(a)  # [1, 2, 3]`,
    ...
    'concept:while_loops': `# Simple Explanation:
A while loop repeats code as long as a condition is true. It is useful when you do not know in advance how many times to loop.

# Intermediate Explanation:
while condition: runs the body until the condition becomes false. Be careful to update something inside the loop, or it will run forever. break exits the loop, and continue skips to the next check.

# In-Depth Explanation:
While loops are common for user input validation, game loops, and reading data until a sentinel value. The condition is checked before each iteration. An else block runs if the loop ends normally without break.
